...
### 需求: CallbackBridge 必须把 Agent 产物回写到对应业务表
...
#### 场景: DP 产物写入 o_assets 和 o_storyboard
...
### 需求: Harness 主控台必须复用业务页面组件

系统 SHALL 要求 Harness 主控台的 7 个步骤子组件复用现有业务页面的 Vue 组件, 不维护第二套 UI。

#### 场景: StageScriptEditor 复用 #/script 的 ScriptTable
...
#### 场景: StageShotImageGrid 复用 #/cornerScape 的批量生图 UI
