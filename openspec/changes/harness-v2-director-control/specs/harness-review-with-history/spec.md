...
### Requirement: 监制 Agent 必须用 LLM 决策通过/打回/升级用户

系统 SHALL 由 SupervisorAgent 调用 LLM 综合判断审核结果, 输出结构化决策。

#### Scenario: 艺术审核失败, 监制决策打回给 DP
...
### Requirement: 用户确认环节必须在关键节点弹出确认窗口
