<template>
  <div class="main" :style="{ height: isElectron ? 'calc(100vh - 32px)' : '100vh' }">
    ...
    <DirectorDock v-if="project?.id" :project-id="project.id" :route-path="route.path" />
  </div>
  <hello />
  <setting />
</template>
...
import DirectorDock from "@/components/harness/DirectorDock.vue";
import harnessWorkbenchStore from "@/stores/harnessWorkbench";
const { refreshRevision } = storeToRefs(harnessWorkbenchStore());
