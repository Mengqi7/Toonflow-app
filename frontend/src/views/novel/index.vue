...
function getNovel() {
  ...
  axios
    ...
    .then((res) => {
      ...
      harnessWorkbench.setVisible(tableData.value.map(item => ({ type: "artifact", id: `artifact:novel-${item.id}`, label: item.chapter })));
    })
    .finally(() => {
      loading.value = false;
    });
...
function handleSelectChange(value: Array<string | number>, context: { selectedRowData: any[] }) {
  ...
  harnessWorkbench.setSelection(
    context.selectedRowData.map(item => ({ type: "artifact", id: `artifact:novel-${item.id}`, label: item.chapter })),
  );
}
