...
watch(selectedIds, ids => {
  const selectedScripts = scripts.value.filter(item => ids.includes(item.id));
  harnessWorkbench.setSelection(selectedScripts.map(item => ({ type: "script", id: `script:${item.id}`, label: item.name })));
  if (selectedScripts.length === 1) harnessWorkbench.setEpisode(selectedScripts[0].id);
}, { deep: true });
// 搜索剧本
async function searchScripts() {
  try {
    ...
    harnessWorkbench.setVisible(scripts.value.map(item => ({ type: "script", id: `script:${item.id}`, label: item.name })));
  } catch (error) {
...
function handleScriptClick(item: Script) {
  ...
  harnessWorkbench.setEpisode(item.id);
  harnessWorkbench.setSelection([{ type: "script", id: `script:${item.id}`, label: item.name }]);
}
// 删除剧本
