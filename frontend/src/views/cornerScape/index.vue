...
import harnessWorkbenchStore, { type HarnessEntityRef } from "@/stores/harnessWorkbench";
...
async function getFilteredData() {
  try {
    ...
    harnessWorkbench.setVisible(dataList.value.map(toHarnessRef));
  } catch (error) {
...
function toHarnessRef(item: DataItem): HarnessEntityRef {
  const type = item.type === "role" ? "character" : item.type === "tool" ? "prop" : "location";
  return { type, id: `${type}:${item.id}`, label: item.name };
}

watch(selectedIds, ids => {
  harnessWorkbench.setSelection(dataList.value.filter(item => ids.includes(item.id)).map(toHarnessRef));
}, { deep: true });
...
async function openDrawer(item: DataItem) {
  ...
  harnessWorkbench.setSelection([toHarnessRef(item)]);
  // 重新获取最新数据（含历史图片）
  try {
    const { data } = await axios.post("/cornerScape/getAllAssets", {
      projectId: project.value?.id,
