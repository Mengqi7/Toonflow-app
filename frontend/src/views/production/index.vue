...
watch(episodesId, value => harnessWorkbench.setEpisode(value || undefined), { immediate: true });
watch(
  () => flowData.value.storyboard,
  items => harnessWorkbench.setVisible((items || []).filter(item => item.id).map((item, index) => ({
    type: "shot",
    id: `shot:${item.id}`,
    label: `镜头 ${index + 1}`,
  }))),
