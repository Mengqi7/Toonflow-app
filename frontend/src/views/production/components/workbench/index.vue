...
function editFootage() {
  axios
    ...
    .then(({ data }) => {
      ...
      harnessWorkbench.setVisible([
        ...initialVideoItems.value.map(item => ({ type: "artifact" as const, id: String(item.id).replace("video-", "artifact:video-"), label: item.name })),
        ...mockAudioItems.value.map(item => ({ type: "artifact" as const, id: String(item.id).replace("audio-", "artifact:audio-"), label: item.name })),
      ]);
    });
}
