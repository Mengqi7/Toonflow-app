...
const harnessWorkbench = harnessWorkbenchStore();
function applyHarnessPatch(event: Event) {
  const patch = (event as CustomEvent).detail;
  if (patch?.domain !== "storyboard") return;
  const id = Number(String(patch.target?.id || "").split(":").pop());
  const target = storyboard.value.find(item => item.id === id);
  if (!target || patch.operation === "remove") return;
  const changes = patch.changes || {};
  if (changes.prompt !== undefined) target.prompt = changes.prompt;
  if (changes.videoDesc !== undefined) target.videoDesc = changes.videoDesc;
  if (changes.duration !== undefined) target.duration = Number(changes.duration);
  if (changes.filePath !== undefined) target.src = changes.filePath;
}
onMounted(() => window.addEventListener("harness:ui-patch", applyHarnessPatch));
onUnmounted(() => window.removeEventListener("harness:ui-patch", applyHarnessPatch));
...
watch(selectedIds, ids => {
  harnessWorkbench.setSelection(
    storyboard.value
      .map((item, index) => ({ item, index }))
      .filter(({ item }) => item.id && ids.includes(item.id))
      .map(({ item, index }) => ({ type: "shot", id: `shot:${item.id}`, label: `镜头 ${index + 1}` })),
  );
}, { deep: true });
...
function editInfo(item: Storyboard) {
  const itemIndex = storyboard.value.findIndex(frame => frame.id === item.id);
  harnessWorkbench.setSelection([{ type: "shot", id: `shot:${item.id}`, label: `镜头 ${itemIndex + 1}` }]);
